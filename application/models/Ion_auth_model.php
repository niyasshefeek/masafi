...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	function get_all_modules($user_id=0){
			$this->db->select("
				'$user_id' as user_id
				,name_mod
				,id_mod
				,IF((SELECT acc.id_module FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod)>0,1,0) as id_mod_checked
				,IFNULL((SELECT acc.can_edit FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod),0) as can_edit
				,IFNULL((SELECT acc.can_save FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod),0) as can_save
				,IFNULL((SELECT acc.can_delete FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod),0) as can_delete
				,IFNULL((SELECT acc.can_upload FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod),0) as can_upload
				,IFNULL((SELECT acc.can_print FROM access_module as acc WHERE '$user_id' = acc.user_id AND acc.id_module = mod.id_mod),0) as can_print
			");
			...
			return $this->db->get("_modules as mod");	
		
		
	} 
	function save_update_privilleges($arr,$id=0){
		
		$del = $this->db->delete('access_module',array(
		'user_id' => $id
		));
		if($del && $arr){
			$this->db->insert_batch('access_module',$arr);
		}
