	...
	$.fn.multiple_emails = function(options) {
		...
		return this.each(function() {
			...
			*/
			...
			//ajax check
			function ajaxcheck (data2) {
				if (settings.ajax.toLowerCase() === "true")
				{
					 ...
					 if(data.success == true){
						  ...
						  $('#error-email').html('<h3 style="color:red"><a href="current-enquiry?i='+data2+'">'+ data2 +'</a> already Exists</h3>');
						  var existingElement2 = $list.find('.email_name[data-email=' + data2.toLowerCase().replace('.', '\\.').replace('@', '\\@') + ']');
						   existingElement2.parent().remove(); refresh_emails(); 
					}
...
(function( $ ){
	...
	$.fn.multiple_phones = function(options) {
		...
		return this.each(function() {
			...
			*/
			function display_email(t, dupEmailCheck) {
				...
				var errorEmails = new Array(); //New array to contain the errors
				var formatsss = "9999|99999|999999|9999999|99999999|999999999|(999)999-9999|999-999-9999|9999999999|99-999999999|99-9999999|999-9999999|99999999999999|99999999999|999999999999|9999999999999";
				
				var pattern = RegExp("^(" + formatsss .replace(/([\(\)])/g, "\\$1")  .replace(/9/g,"\\d") + ")$");
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
				        ...
				        if (arr[i] && arr[i].length > 0) {
							new function () {
								var existingElement = $list.find('.email_name[data-email=' + arr[i].toLowerCase() + ']');
								existingElement.css('font-weight', 'bold');
								setTimeout(function() { existingElement.css('font-weight', ''); }, 1500);
							}(); // Use a IIFE function to create a new scope so existingElement won't be overriden
						}
			...
			function ajaxcheck (data2) {
				if (settings.ajax.toLowerCase() === "true")
				{
					 ...
					 if(data.success == true){
						  ...
						  $('#error-mob').html('<h3 style="color:red"><a href="current-enquiry?i='+data2+'">'+ data2 +'</a> already Exists</h3>');
						  var existingElement2 = $list.find('.email_name[data-email=' + data2.toLowerCase() + ']');
						   existingElement2.parent().remove(); refresh_emails(); 
					}
